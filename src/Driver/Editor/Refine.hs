...
offerRefinements uri diag meta = do
  ...
  case metaGoal meta of
    VPi Binder{var} _ -> do
      let
        spaces = metaSpan - 1
        text = showsPrec (maybe 0 contextPrecedence (metaContext meta)) lambda ""
        lambda = VFn var $ \_ -> valueVar (Intro "_")
      pure [offerText "Introduce λ-expression" uri diag version (T.pack text <> T.replicate spaces (T.singleton ' ')) range]
